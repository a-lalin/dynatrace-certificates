...
#OneAgent #ActiveGate #OpenTelemetry #Business_Events #API #Kubernetes #Hub_Extensions
...
It's main job is to collect data from the hosts it's installed on and send that data to the Dynatrace platform via [[#ActiveGate]]. Installed on a host or a server.
...
Each OneAgent deployment process requires selecting a [[3. Infrastructure#Monitoring Modes|Monitoring Mode]], HostGroup, Network Zone and some other parameters.
## [ActiveGate](https://docs.dynatrace.com/docs/ingest-from/dynatrace-activegate)
To ensure the [[#OneAgent]] data is sent securely, most companies set up firewall rules for every server that has the OneAgent installed. ActiveGate **receives OneAgent data/traffic**, compresses and encrypts it, and **sends to Dynatrace**. It **only talks outward** – nothing can come in. That helps keep servers safe from unwanted visitors or attacks.
* Acts as a proxy between OneAgents and Dynatrace Clusters
* Many OneAgents can speak to one ActiveGate
* It is also capable of performing monitoring tasks using [[#Dynatrace API]]
...
Of course, if you want deeper visibility, like logs, process metrics, services and traces, you’ll still want to install OneAgent. But these integrations are a great addition, especially for distributed or hybrid cloud environments.

  

## Logs Ingestion, Masking, and Access

Here is another way Dynatrace and gather data beyond the OneAgent.

Select the plus (+) icons to explore how Log Ingestion allows Dynatrace to collect log data from multiple different sources:
...
Dynatrace empowers us to take full control of our log data. By customizing ingestion and retention rules, we optimize our environments for efficiency, security, and streamlined analysis!